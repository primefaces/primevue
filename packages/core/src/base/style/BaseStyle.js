...
export const getBaseStyleInstance = (Theme, dt) => ({
    name: 'base',
    Theme,
    dt,
    css,
    ...
    load(style, options = {}, transform = (cs) => cs) {
        const computedStyle = transform(resolve(style, { dt: this.dt }));
    ...
    loadStyle(options = {}, style = '') {
        return this.load(this.style, options, (computedStyle = '') => this.Theme.transformCSS(options.name || this.name, `${computedStyle}${style}`));
    },
    ...
    getStyleSheet(extendedCSS = '', params = {}, props = {}) {
        if (this.css) {
            ...
            const _style = transformCSS(`${_css}${extendedCSS}`, params);
    ...
    getThemeStyleSheet(params, props = {}) {
        ...
        if (this.style) {
            ...
            const _css = resolve(this.style, { dt: this.dt });
    ...
    bindInstance(BaseStyle) {
        return { ...this, Theme: BaseStyle.Theme, dt: BaseStyle.dt };
    }
});

export default getBaseStyleInstance(Theme, dt);
