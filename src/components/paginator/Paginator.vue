    ...
    <div v-if="alwaysShow ? true : pageLinks && pageLinks.length > 1">
        <div v-for="(value, key) in templateItems" :key="key" ref="paginator" class="p-paginator p-component" :class="getPaginatorClasses(key)">
            ...
            <template v-for="item in value" :key="item">
                ...
                <PrevPageLink v-else-if="item === 'PrevPageLink'"
                :aria-label="getAriaLabel('previousPageLabel')" @click="changePageToPrev($event)" :disabled="isFirstPage || empty" />
                <NextPageLink v-else-if="item === 'NextPageLink'"
                :aria-label="getAriaLabel('nextPageLabel')" @click="changePageToNext($event)" :disabled="isLastPage || empty" />
                <LastPageLink v-else-if="item === 'LastPageLink'"
                :aria-label="getAriaLabel('lastPageLabel')" @click="changePageToLast($event)" :disabled="isLastPage || empty" />
                ...
                <CurrentPageReport v-else-if="item === 'CurrentPageReport'"
                aria-live="polite"
                :template="currentPageReportTemplate" :currentPage="currentPage" :page="page" :pageCount="pageCount" :first="d_first" :rows="d_rows" :totalRecords="totalRecords" />
                <RowsPerPageDropdown v-else-if="item === 'RowsPerPageDropdown' && rowsPerPageOptions"
                :aria-label="getAriaLabel('rowsPerPageDropdownLabel')" :rows="d_rows" :options="rowsPerPageOptions" @rows-change="onRowChange($event)" :disabled="empty" />
                <JumpToPageDropdown v-else-if="item === 'JumpToPageDropdown'"
                :aria-label="getAriaLabel('jumpToPageLabel')" :page="page" :pageCount="pageCount" @page-change="changePage($event)" :disabled="empty" />
                <JumpToPageInput v-else-if="item === 'JumpToPageInput'"
                :aria-label="getAriaLabel('jumpToPageLabel')" :page="currentPage" @page-change="changePage($event)" :disabled="empty" />
            </template>
