...
export default {
    ...
    methods: {
        ...
        getOptionRenderKey(option, index) {
            return this.dataKey ? ObjectUtils.resolveFieldData(option, this.dataKey) : this.getOptionLabel(option) + index; // Ensure uniqueness of key if no key specified
        },
        ...
        visibleOptions() {
            if (this.filterValue) {
                if (this.optionGroupLabel) {
                    ...
                    const results = FilterService.filter(this.options, this.searchFields, this.filterValue, this.filterMatchMode, this.filterLocale);
                    console.log(results);
                    return results;
                }
            }
