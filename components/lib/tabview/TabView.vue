<template>
    <div :class="cx('root')" v-bind="ptm('root')" data-pc-name="tabview">
        <div :class="cx('navContainer')" v-bind="ptm('navContainer')">
            <button v-if="scrollable && !isPrevButtonDisabled" ref="prevBtn" v-ripple type="button"
                :class="cx('previousButton')" :tabindex="tabindex" :aria-label="prevButtonAriaLabel"
                @click="onPrevButtonClick" v-bind="{ ...previousButtonProps, ...ptm('previousButton') }"
                data-pc-group-section="navbutton">
                <slot name="previcon">
                    <component :is="prevIcon ? 'span' : 'ChevronLeftIcon'" aria-hidden="true" :class="prevIcon"
            ...
            <div ref="content" :class="cx('navContent')" @scroll="onScroll" v-bind="ptm('navContent')">
                <ul ref="nav" :class="cx('nav')" role="tablist" v-bind="ptm('nav')">
                    <li v-for="(tab, index) of tabs" :key="getKey(tab, index)" :style="getTabProp(tab, 'headerStyle')"
                        :class="cx('tab.header', { tab, index })" role="presentation"
                        v-bind="{ ...getTabProp(tab, 'headerProps'), ...getTabPT(tab, 'root', index), ...getTabPT(tab, 'header', index) }"
                        data-pc-name="tabpanel" :data-p-highlight="d_activeIndex === index"
                        :data-p-disabled="getTabProp(tab, 'disabled')" :data-pc-index="index"
                        :data-p-active="d_activeIndex === index">
                        <a :id="getTabHeaderActionId(index)" v-ripple :class="cx('tab.headerAction')"
                            :tabindex="getTabProp(tab, 'disabled') || !isTabActive(index) ? -1 : tabindex" role="tab"
                            :aria-disabled="getTabProp(tab, 'disabled')" :aria-selected="isTabActive(index)"
                            :aria-controls="getTabContentId(index)" @click="onTabClick($event, tab, index)"
                            @keydown="onTabKeyDown($event, tab, index)"
                            v-bind="{ ...getTabProp(tab, 'headerActionProps'), ...getTabPT(tab, 'headerAction', index) }">
                            <span v-if="tab.props && tab.props.header" :class="cx('tab.headerTitle')"
                                v-bind="getTabPT(tab, 'headerTitle', index)">{{ tab.props.header }}</span>
                            <component v-if="tab.children && tab.children.header" :is="tab.children.header"></component>
                        </a>
                    </li>
                    <li ref="inkbar" :class="cx('inkbar')" role="presentation" aria-hidden="true" v-bind="ptm('inkbar')">
                    </li>
                </ul>
            </div>
            <button v-if="scrollable && !isNextButtonDisabled" ref="nextBtn" v-ripple type="button"
                :class="cx('nextButton')" :tabindex="tabindex" :aria-label="nextButtonAriaLabel" @click="onNextButtonClick"
                v-bind="{ ...nextButtonProps, ...ptm('nextButton') }" data-pc-group-section="navbutton">
                <slot name="nexticon">
                    <component :is="nextIcon ? 'span' : 'ChevronRightIcon'" aria-hidden="true" :class="nextIcon"
                        v-bind="ptm('nextIcon')" />
                </slot>
            </button>
        </div>
        <div :class="cx('panelContainer')" v-bind="ptm('panelContainer')">
            <template v-for="(tab, index) of tabs" :key="getKey(tab, index)">
                <div v-if="lazy ? isTabActive(index) : true" v-show="lazy ? true : isTabActive(index)"
                    :id="getTabContentId(index)" :style="getTabProp(tab, 'contentStyle')"
                    :class="cx('tab.content', { tab })" role="tabpanel" :aria-labelledby="getTabHeaderActionId(index)"
                    v-bind="{ ...getTabProp(tab, 'contentProps'), ...getTabPT(tab, 'root', index), ...getTabPT(tab, 'content', index) }"
                    data-pc-name="tabpanel" :data-pc-index="index" :data-p-active="d_activeIndex === index">
