    ...
    <Paginator unstyled>
        <template #container="{ first, last, rows, page, pageCount, pageLinks, totalRecords, changePageCallback, firstPageCallback, lastPageCallback, prevPageCallback, nextPageCallback, rowChangeCallback }">
            <div class="flex flex-wrap gap-2 items-center justify-center">
                ...
                <SecondaryButton text rounded @click="prevPageCallback">
                    <template #icon>
                        <AngleLeftIcon />
                    </template>
                </SecondaryButton>
                <div class="items-center justify-center gap-2 hidden sm:flex">
                    <SecondaryButton v-for="pageLink of pageLinks" :key="page" :text="page !== pageLink" rounded @click="() => changePageCallback(pageLink)" :class="['shrink-0 min-w-10 h-10', { 'bg-highlight!': page === pageLink }]"
                        >{{ pageLink }}
                    </SecondaryButton>
                </div>
                <SecondaryButton text rounded @click="nextPageCallback">
                    <template #icon>
                        <AngleRightIcon />
                    </template>
                </SecondaryButton>
